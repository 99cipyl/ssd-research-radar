...
- 只有状态为 `professional` 且全部字段、证据原句、SSD 层级与数字声明通过校验的新增/更新，才会进入 Live、完整 Feed、历史分卷和 WebSub 推送。网络、配额或校验失败时，事件留在 outbox，按退避窗口自动重试；
- 历史基线先提供不复制原文乱文本的“证据受限”页面，再按最新优先分批回填专业简报。证据等级会区分官方网页正文、官方摘要、来源摘要/摘录和仅元数据。

OPML 中已经把订阅分成两个文件夹：

- “SSD 即时更新（开启通知）”：用于手机新内容提醒；
- “SSD 历史归档（建议关闭通知）”：用于浏览旧资料，建议导入后标为已读并关闭通知，避免初次导入产生大量提醒。
...
“全部历史”指源站或公开 API 目前仍提供的历史：

- FAST、SAFARI、NVM Express 和 OpenAlex 可以分页回溯；
...
- OpenAlex 免费接口无法按“索引更新时间”拉取，日常同步会重扫最近一年，每 30 天还会自动重扫完整关键词历史以覆盖延迟收录；仍可能晚于 Scholar/IEEE 邮件几天。

如有 OpenAlex API Key，可以在运行环境中设置 `OPENALEX_API_KEY`；脚本会自动使用它，但不会把密钥写入数据库或报告。
...
公开仓库的 GitHub Actions 会约每 15 分钟检查 RSS、WordPress 和规范页；FAST 与 OpenAlex 每天检查一次，并每月做一次完整学术回扫。另有本机 Codex Heartbeat 作为故障提醒与备用检查。

自动检查读取 `reports/latest.json`：

- `new_count > 0`：发送标题、来源、日期、站内专业简报链接、核心思想和工程相关性；
- `failures` 非空：报告失败来源，下一次会自动重试；
- 没有新增也没有故障：保持静默。
