...
- OpenAlex 无法按“索引更新时间”做可靠增量，因此首次基线以后每天重扫最近一年。每页固定为 50 条，并在同一轮内执行有界的网络、JSON 完整性和返回结构重试；同时校验稳定 Work ID、游标连续性、跨页去重和 `meta.count`，并用首屏计数提前拒绝必然超过安全页数的查询，避免无意义地耗尽配额。跨页不变量异常时会从该查询首页完整重拉一次；仍不完整时整次 OpenAlex 快照失败关闭，不会把已取到的半批数据当作成功。云端不会再做 25 年关键词月度全扫，以免超出免费搜索预算后在末页限流并丢掉整批结果；月度 `--full` 只用于有界的 FAST TOC 回扫。发布日期早于滚动窗口、但一年后才被 OpenAlex 补录的论文仍可能漏掉，因此 Scholar/IEEE 邮件提醒保留为补漏层。
...
- `new_count` / `updated_count`：持久化 outbox 中已经通过 `professional` 门禁、当前可安全推送的事件；大于 0 时发送标题、来源、日期、站内专业简报链接、核心思想和工程相关性；
- `run_detected_new_count` / `run_detected_updated_count`：本轮实际成功检查的来源生成的“相关、在历史窗口内、可通知”候选事件数；它位于专业简报门禁之前，不等于所有写入数据库的原始变化，也不能替代专业简报门禁；
- `source_failures`：来源本轮检查未完成，因此该来源的新增/更新状态未知；通知不得把这种情况写成“本次无新增”；旧数据不受影响，下次自动重试；
- `brief_failures`：资料已经发现，但专业简报没有通过模型或证据校验，继续留在后台重试且不得进入任何订阅源；兼容字段 `failures` 是前两类的合集；
- `source_checks` 与 `source_attempted_count` / `source_success_count` / `source_skipped_count`：区分真实成功检查、复用新鲜缓存和失败，避免把跳过误报为上游恢复；
- 没有新增也没有故障：保持静默。
